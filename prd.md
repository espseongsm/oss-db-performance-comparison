...
- 리포트 본문은 인사이트 중심으로 유지하고 네 작업의 실행 코드·SQL 발췌는 넣지 않는다. 행 수 × 워크로드 선택 가이드와 로컬 Parquet/Snowflake 적재 데이터의 선택 기준을 추가한다. 평균·표본 SD에 근거한 복수 후보, 데이터 이동/결과 다운로드 제외, 비용 미측정 및 정확한 전환 행 수의 불확실성을 명시한다.
...
### CLI integration after merging main

The shared `main.py` entry point preserves all four commands: `financebench`, `warehouse-sweep`, `warehouse-frames`, and `frames`. Snowflake and vector optional dependencies coexist in the validated lockfile.

```mermaid
flowchart LR
  CLI[main.py] --> F[financebench]
  CLI --> S[warehouse-sweep]
