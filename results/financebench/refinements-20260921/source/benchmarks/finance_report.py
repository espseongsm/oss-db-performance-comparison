...
def report(out):
    ...
    for entry in status:
        if entry["status"] != "complete":
            continue
        engine = entry["engine"]
        meta = json.loads((out / engine / "metadata.json").read_text())
        details.append(meta)
        for mode in ["global", "document-filter"]:
            measurements = read_jsonl(out / engine / f"{mode}.jsonl")
            expected = (
    ...
    if rows:
        ...
        for r in rows:
            lines.append(
                f"| {r['engine']} | {r['mode']} | {r['p50_ms']:.2f} | {r['p95_ms']:.2f} | {r['ann_recall']:.1%} | {r['tie_aware_ann_recall']:.1%} | {r['evidence_hit']:.1%} |"
            )
    lines += [
        "",
    ...
    lines += [
        f"- {r['engine']}: {r['status']}; see `{r['engine']}.log`." for r in status
    ]
    lines += ["", "### Versions and plans", ""]
    for meta in details:
        lines += [
            f"- {meta['engine']}: {meta['version']}; load + index {meta['load_and_index_seconds']:.2f}s. Execution plans/settings in `{meta['engine']}/metadata.json`."
        ]
    lines += [
        "",
        "## Reproduce",
