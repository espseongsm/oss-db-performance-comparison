...
class DuckDBBackend:
    def __init__(self, vectors, chunks, directory, ann=True, prefilter=False):
        import duckdb
    ...
    def sql(self, vector, doc=None):
        ...
        source = "items" if doc is None or not self.prefilter else "candidates"
        prefix = (
            ""
            if source == "items"
            else f"WITH candidates AS MATERIALIZED (SELECT id, embedding FROM items{where}) "
        )
        if source == "candidates":
            where = ""
        return f"{prefix}SELECT id FROM {source}{where} ORDER BY array_cosine_distance(embedding, {vector_literal(vector)}::FLOAT[{self.dim}]) LIMIT 10"
...
class ClickHouseBackend:
    def __init__(self, vectors, chunks, directory, prefilter=False):
    ...
    def sql(self, vector, doc=None):
        where = "" if doc is None else " WHERE doc='" + doc.replace("'", "\\'") + "'"
        strategy = (
            ", vector_search_filter_strategy='prefilter'"
            if doc is not None and self.prefilter
            else ""
        )
        return f"SELECT id FROM fb_items{where} ORDER BY cosineDistance(embedding, {vector_literal(vector)}) LIMIT 10 SETTINGS max_threads=4, hnsw_candidate_list_size_for_search=128{strategy}"
