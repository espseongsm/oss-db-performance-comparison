...
def backend(engine, vectors, chunks, directory):
    ...
    if engine.startswith("duckdb"):
        return DuckDBBackend(vectors, chunks, directory, ann=engine.endswith("hnsw"))
...
def run(out, engines, repeats):
    ...
    for engine in engines:
        if engine in SERVERS:
            image = subprocess.run(
                COMPOSE + ["images", "--format", "json", engine],
                capture_output=True,
                text=True,
                check=False,
            )
    ...
    for engine in engines:
        try:
            if engine in SERVERS:
                subprocess.run(COMPOSE + ["up", "-d", engine], check=True)
                wait_server(engine)
            ...
            if engine in SERVERS:
                with (out / f"{engine}-server.log").open("w") as log:
                    subprocess.run(
                        COMPOSE + ["logs", engine], stdout=log, stderr=subprocess.STDOUT
                    )
                subprocess.run(COMPOSE + ["stop", engine], check=False)
