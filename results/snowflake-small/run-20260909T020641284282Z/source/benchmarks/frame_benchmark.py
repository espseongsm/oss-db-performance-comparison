...
def parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="저장된 Snowflake connection 이름. 지정하면 원격 warehouse 측정도 추가합니다.",
    )
    parser.add_argument(
        "--snowflake-password-prompt",
        action="store_true",
        help="터미널에서 비밀번호를 입력합니다. keychain 옵션과 함께 쓰면 저장 항목을 갱신합니다.",
    )
    parser.add_argument(
        "--snowflake-keychain",
        action="store_true",
        help="macOS 키체인 사용. 최초 1회 터미널 입력 후 인증에 성공하면 저장합니다.",
    )
    args = parser.parse_args(argv)
    if (args.snowflake_password_prompt or args.snowflake_keychain) and not args.snowflake_connection:
        parser.error("Snowflake 인증 옵션에는 --snowflake-connection이 필요합니다.")
    if args.pilot:
...
def main(argv: list[str] | None = None) -> int:
    ...
    def measure(case: dict, profile: bool) -> dict:
        if case["engine"] == "snowflake":
            return snowflake.run(case, args.runs // args.blocks, profile)
        return run_worker(args, case, datasets[case["rows"]], profile)
    ...
    try:
        if args.snowflake_connection:
            ...
            authentication = {"prompt_password": True} if args.snowflake_password_prompt else {}
            if args.snowflake_keychain:
                authentication["use_keychain"] = True
            snowflake = SnowflakeBackend(
                args.snowflake_connection, args.timeout_seconds, **authentication
            )
            manifest["snowflake"] = snowflake.metadata
            manifest["snowflake_scope"] = (
                "Preloaded remote tables -> SQL -> fetch_pandas_all -> int64 DataFrame. "
                "Upload/COPY separately timed; result reuse disabled; warehouse data cache uncontrolled. "
                "Persistent remote session; memory and cpu_ms never represent server memory/CPU."
            )
            save_json(output / "manifest.json", manifest)
        datasets = {}
        for rows in args.sizes:
            ...
            if snowflake is not None:
                print(f"Snowflake 업로드·적재: {rows:,}행", flush=True)
                snowflake.load(datasets[rows])
                save_json(output / "snowflake-setup.json", snowflake.setup_records)
        save_json(output / "datasets.json", list(datasets.values()))
        rng = random.Random(args.seed)
        cases = list(itertools.product(args.sizes, ("parquet", "memory"), WORKLOADS))
        if snowflake is not None:
            cases.extend(itertools.product(args.sizes, ("warehouse",), WORKLOADS))
        expected_groups = len(args.sizes) * len(WORKLOADS) * (5 if snowflake else 4)
        for block in range(1, args.blocks + 1):
            rng.shuffle(cases)
            for rows, mode, workload in cases:
                engines = ["snowflake"] if mode == "warehouse" else ["pandas", "duckdb"]
                rng.shuffle(engines)
        ...
        if len(counts) != expected_groups or not counts.eq(args.runs).all():
        ...
        comparison = compare(raw_frame.loc[raw_frame.engine != "snowflake"], args.seed)
        summary.to_csv(output / "summary.csv", index=False)
        summary.to_json(output / "summary.json", orient="records", indent=2)
        comparison.to_csv(output / "comparison.csv", index=False)
        if snowflake is not None:
            manifest["snowflake_history_errors"] = snowflake.history_errors
        manifest.update(
            {
                "status": "complete",
