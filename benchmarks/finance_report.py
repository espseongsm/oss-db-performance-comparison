...
def report(out):
    ...
    if rows:
        ...
        for ax in axes:
            ax.set_yticks(
                ...
                ["cortex (cloud / HTTPS)" if e == "cortex" else e for e in engines],
            )
            ax.invert_yaxis()
        ...
        fig.legend(
            *axes[0].get_legend_handles_labels(), loc="outside lower center", ncol=2
        )
        fig.suptitle(
            f"FinanceBench | {manifest['chunk_count']:,} chunks | {manifest['questions']} queries | concurrency 1"
            + (
                "\nLocal and cloud deployment paths have different hardware/network overhead"
                if has_cortex
                else ""
            )
        )
    ...
    lines += [
        ...
        (
            ...
            if has_cortex
            else "- Snowflake SQL and Cortex Search: not measured in this local run; credentials/configuration and a separate cloud-cost measurement are required."
        ),
