...
def report_text(summary, manifest, calls, qa, images):
    ...
    lines.extend(
        [
            ...
            f"| Python 런타임 | {runtime['platform']}; Python {runtime['python'].split()[0]} |",
            ...
            f"- 측정에 연결된 child query history: {qa['history_found']:,}/{qa['history_expected']:,}개 확보, 누락 {qa['history_missing']:,}개. 서버 세부 지표와 직접 측정 시간을 구분한다.",
